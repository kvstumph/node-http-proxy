...
common.setupOutgoing = function(outgoing, options, req, forward) {
  ...
  var headersWithAllUppercaseKeysPreserved = {};
  for (var i = 0; i < req.rawHeaders.length - 1; i += 2) {
    var key = req.rawHeaders[i];
    if(key === key.toUpperCase()) {
      headersWithAllUppercaseKeysPreserved[key] = req.rawHeaders[i + 1];
    }
    else {
      headersWithAllUppercaseKeysPreserved[key.toLowerCase()] = req.rawHeaders[i + 1];
    }
  }
  outgoing.headers = extend({}, headersWithAllUppercaseKeysPreserved);
